...
type CustomMetricLister interface {
	...
	List(selector labels.Selector) (ret []*v1.CustomMetric, err error)
	// Get retrieves the CustomMetric from the index for a given name.
	Get(name string) (*v1.CustomMetric, error)
...
// List lists all CustomMetrics in the indexer.
func (s *customMetricLister) List(selector labels.Selector) (ret []*v1.CustomMetric, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.CustomMetric))
	})
	return ret, err
}

// Get retrieves the CustomMetric from the index for a given name.
func (s *customMetricLister) Get(name string) (*v1.CustomMetric, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("custommetric"), name)
	}
	return obj.(*v1.CustomMetric), nil
}
